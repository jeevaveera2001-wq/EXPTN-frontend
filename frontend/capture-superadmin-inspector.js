...
(async () => {
  ...
  await page.screenshot({ path: path.join(ARTIFACTS_DIR, 'superadmin_tab4_owner_requests_live.png'), fullPage: false });
  console.log('📸 Captured superadmin_tab4_owner_requests_live.png');
  ...
  // Also switch to Tab 5 (Properties & Fleet) and click Inspect
  await page.evaluate(() => {
    const tabs = Array.from(document.querySelectorAll('button'));
    const propTab = tabs.find(b => b.textContent && b.textContent.includes('Properties'));
    if (propTab) propTab.click();
  });
  await new Promise(r => setTimeout(r, 1500));

  // Click Vehicles sub-tab in Properties
  await page.evaluate(() => {
    const tabs = Array.from(document.querySelectorAll('button'));
    const vehSubTab = tabs.find(b => b.textContent && b.textContent.includes('Vehicles'));
    if (vehSubTab) vehSubTab.click();
  });
  await new Promise(r => setTimeout(r, 1500));

  await page.screenshot({ path: path.join(ARTIFACTS_DIR, 'superadmin_tab5_vehicles_fleet_live.png'), fullPage: false });
  console.log('📸 Captured superadmin_tab5_vehicles_fleet_live.png');
